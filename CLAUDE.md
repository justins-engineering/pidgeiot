...
- **Device-facing shadow report-back and telemetry ingestion** (`objects/pigeons.rs`, added 2026-07-15): both share device auth via `is_authorized_device()` (factored out of what used to be three near-identical checks duplicated across `get_shadow_device`/`report_shadow_device`/`report_telemetry_device` — bearer token vs. this pigeon's own `device_public_key`, no ACL). `POST /device/pigeons/:id/shadow` → DO-internal `/pigeon/device/shadow/report` → `report_shadow_device`: a device confirms it applied `target_config` by writing its own `current_config`/`current_version` into `pigeon_shadow`; like the dashboard's `PUT /pigeons/:id/shadow`, the gateway route also best-effort syncs the result to Postgres via the existing `update_shadow_pg_db`, so Postgres's mirrored shadow state doesn't go stale for pigeons that report back on their own. `POST /device/pigeons/:id/telemetry` → DO-internal `/pigeon/device/telemetry` → `report_telemetry_device`: body is a flat JSON object of string key/value pairs, upserted one row per key into a new `pigeon_telemetry` table (`key` `PRIMARY KEY`, `value`, `reported_at`) — latest-value-per-key, not a time-series log; no read route or Postgres mirror yet (a history/range-query table is a deliberately deferred follow-up).
