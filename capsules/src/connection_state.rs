...
/// `fancier::components::firmware_modal::extract_firmware_target`. `None`
/// covers both "not valid JSON" and "no `telemetry_interval` key set".
...
pub fn latest_of(
  times: impl IntoIterator<Item = Option<OffsetDateTime>>,
...
/// are trustworthy (see `has_never_reported`, used by `fancier`'s
/// `PigeonView` to drop a never-confirmed shadow's `updated_at` before
/// merging, and dovecote's `evaluate_scheduled_alerts`) and hand this
...
/// the flock pigeon-list makes for this feature (see `fancier`'s
/// `views/pigeons.rs`): no per-pigeon fan-out, no shadow/log signals, just
/// the newest `reported_at` per `pigeon_id` within whatever window the
/// caller queried.
...
mod tests {
  ...
  fn never_reported_false_once_config_is_non_trivial() {
    ...
    assert!(!has_never_reported(
      0,
      &config(r#"{"telemetry_interval":60}"#)
    ));
  }
  ...
  fn latest_of_picks_the_newest_present_signal() {
    ...
    assert_eq!(
      latest_of([a, b, c]),
      Some(datetime!(2026-07-18 12:00:00 UTC))
    );
  }
  ...
  fn classify_beyond_5x_interval_is_offline() {
    ...
    assert_eq!(
      classify(Some(seen), Some(60), now),
      ConnectionState::Offline
    );
  }
  ...
  fn classify_falls_back_to_fixed_thresholds_with_no_interval() {
    ...
    assert_eq!(
      classify(Some(just_online), None, now),
      ConnectionState::Online
    );
    assert_eq!(
      classify(Some(just_stale), None, now),
      ConnectionState::Stale
    );
    assert_eq!(
      classify(Some(just_offline), None, now),
      ConnectionState::Offline
    );
  }
  ...
  fn classify_treats_future_timestamps_as_now() {
    ...
    assert_eq!(
      classify(Some(future), Some(60), now),
      ConnectionState::Online
    );
  }
