...
# dependency graph -- so the page can never drift stale the way a
# hand-maintained list would. Run by fancier/scripts/build-release.sh on
# every release build. The checked-in output files
# (fancier/generated/oss-licenses.md, dovecote/generated/oss-licenses.md)
# are committed snapshots so plain `cargo check`/`dx serve` keep working
# without cargo-about installed -- only a release build regenerates them.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
# better than omission for an open platform (see CLAUDE.md/open-source page
# design notes). Both share one about.toml -- the accepted-license policy
# is a whole-project decision, not a per-crate one.
...
# about a prefix it has no way to receive (cargo-about's CLI only ever
# feeds it the fixed license-report JSON, no extra template variables).
