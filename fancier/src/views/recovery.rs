...
#[component]
pub fn RecoveryFlow(flow: Option<String>) -> Element {
  // 1. Change the error arm of the Result to `Element` to support polymorphic error rendering
  let mut flow_state =
    use_signal(|| None::<Result<ory_kratos_client_wasm::models::RecoveryFlow, Element>>);

  let mut session = use_context::<Session>();
  let nav = use_navigator();

  // 2. Initialize or fetch the recovery flow
  use_future(move || {
    ...
    async move {
      ...
      let result = match flow_param {
        Some(id) => match get_recovery_flow(&config, &id, None).await {
          Ok(res) => Ok(res),
        ...
        None => match create_browser_recovery_flow(&config, None).await {
          Ok(res) => Ok(res),
          Err(ory_kratos_client_wasm::apis::Error::ResponseError(res)) => {
            Err(res.view_response_content())
          }
          ...
          Ok(resp_value) => {
            ...
            if (200..300).contains(&status) {
              ...
              match JsFuture::from(resp.json().unwrap()).await {
                Ok(json_val) => {
                  match serde_wasm_bindgen::from_value::<ory_kratos_client_wasm::models::RecoveryFlow>(
                    json_val,
                  ) {
  ...
  match &*flow_state.read() {
    Some(Ok(res)) => {
      ...
      rsx! {
        h1 { class: "text-center text-2xl mt-10", "Account Recovery" }
    ...
    None => {
      rsx! {
        div { class: "flex justify-center mt-10",
          p { class: "animate-pulse", "Loading recovery flow..." }
        }
      }
    }
  }
