...
pub fn RegisterFlow(flow: Option<String>) -> Element {
  let mut flow_state =
    use_signal(|| None::<Result<ory_kratos_client_wasm::models::RegistrationFlow, Element>>);
  ...
  // 1. Initialize or fetch the registration flow
  use_future(move || {
        ...
        match JsFuture::from(window.fetch_with_request(&req_builder)).await {
          Ok(resp_value) => {
            ...
            if (200..300).contains(&status) {
              ...
              match JsFuture::from(resp.json().unwrap()).await {
                Ok(json_val) => {
                  match serde_wasm_bindgen::from_value::<
                    ory_kratos_client_wasm::models::RegistrationFlow,
                  >(json_val)
                  {
              ...
              // Extract error before resetting the registration pipeline
