...
pub fn VerificationFlow(flow: Option<String>) -> Element {
  ...
  let mut flow_state =
    use_signal(|| None::<Result<ory_kratos_client_wasm::models::VerificationFlow, Element>>);

  let mut session = use_context::<Session>();
  let nav = use_navigator();

  // 2. Initialize or fetch the verification flow
  use_future(move || {
    ...
    async move {
      ...
      let result = match flow_param {
        Some(id) => match get_verification_flow(&config, &id, None).await {
          Ok(res) => Ok(res),
        ...
        None => match create_browser_verification_flow(&config, None).await {
          Ok(res) => Ok(res),
          Err(ory_kratos_client_wasm::apis::Error::ResponseError(res)) => {
            Err(res.view_response_content())
          }
              ...
              match JsFuture::from(resp.json().unwrap()).await {
                Ok(json_val) => {
                  match serde_wasm_bindgen::from_value::<
                    ory_kratos_client_wasm::models::VerificationFlow,
                  >(json_val)
                  {
