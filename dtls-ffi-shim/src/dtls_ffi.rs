...
//!
//! # SAFETY -- read this before integrating
//!
...
//! here is on the pre-authentication attack surface. Checklist for a reviewer:
//!
//! 1. **No panics on attacker input.** `dtlsv1_listen` must never panic or
//!    abort regardless of what bytes are sitting on the `Ssl`'s read side --
//!    garbage, truncated records, a ClientHello with a bogus or replayed
//!    cookie, or an empty datagram. It only ever surfaces `DtlsShimError` or a
//!    `ListenOutcome`; every OpenSSL return code is matched explicitly with no
//!    `unreachable!()`/`.unwrap()` on values that originate from the C side.
//!    Covered by `test_garbage_datagram_does_not_panic` and
//!    `test_truncated_client_hello_does_not_panic`.
//! 2. **`DTLSv1_listen` operates statelessly on purpose** (that's the whole
//!    point of the cookie exchange: reject bad handshakes before allocating
//!    per-connection state). Nothing in this module allocates connection state
//!    on the caller's behalf before a valid cookie is seen. Callers must not
//!    either -- don't create the full application-level `Pigeon`/connection
//!    context until `dtlsv1_listen` returns `ListenOutcome::Accepted`.
//! 3. **Peer-address auto-discovery from `BIO_ADDR` does not work through
//!    rust-openssl's normal transport bridge.** `SslStream`/`Ssl::accept` in
//!    the safe `openssl` crate drive I/O through a generic `Read + Write`
//!    adapter (see rust-openssl's private `bio.rs`), not a real
//!    `BIO_s_datagram`. That adapter has no notion of "peer address" for
...
//!    normal rust-openssl way. **This is expected, not a bug** -- see
//!    `test_listen_over_readwrite_bridge_never_populates_peer` for the
//!    coverage that pins this down. Practically: whoever builds the demux
//!    layer around this shim must already know which socket address a
//!    datagram came from via their own `UdpSocket::recv_from` (or equivalent)
//!    *before* routing bytes into a specific `Ssl` object -- it cannot lean on
//!    this shim to learn the peer.
//! 4. **Amplification / redirection warning, straight from `DTLSv1_listen(3)`:**
//!    "It is essential that the calling code connects the underlying socket to
//!    the peer after making use of `DTLSv1_listen()`... failing to \[do so\]
//!    means that any host on the network can cause outgoing DTLS traffic to be
//!    redirected to it." This module has no way to enforce that from inside
//!    the shim -- it is a hard requirement on the caller's transport-wiring
//!    code, called out here because it is easy to miss and does not fail
//!    loudly if skipped.
//! 5. **Version gate is a runtime check against the linked library, not a
//!    build-time `cfg`.** `DTLSv1_listen`'s `BIO_ADDR *` parameter (and its
//!    return-code contract) is an OpenSSL >= 1.1.0 ABI; earlier releases used
//!    a raw `struct sockaddr *` there. `openssl-sys`'s own build-time version
...
/// assumes. See the module-level SAFETY note #5.
...
/// module's assumed `DTLSv1_listen` ABI (see SAFETY note #5). Cheap (one
/// FFI call to read a version constant); safe to call on every
/// `dtlsv1_listen` invocation, which it is.
...
pub fn dtls_get_timeout(ssl: &SslRef) -> Option<Duration> {
  let mut tv: libc::timeval = unsafe { std::mem::zeroed() };
  // SAFETY: `ssl.as_ptr()` is a valid, non-null `*mut SSL` for the lifetime
  // of this call (borrowed from `&SslRef`, which cannot outlive its owning
  // `Ssl`/`SslStream`). `&mut tv` is a valid, correctly-sized, correctly-
...
/// not retry.
pub fn dtls_handle_timeout(ssl: &SslRef) -> Result<HandleTimeoutOutcome, DtlsShimError> {
...
impl BioAddr {
  ...
  /// Best-effort extraction into a `std::net::SocketAddr`. See module
  /// SAFETY note #3 for why this is `None` in the common case where `ssl`'s
  /// transport is rust-openssl's ordinary `Read + Write` bridge rather than
  /// a native datagram BIO -- that is expected, not a bug in this function.
  fn to_socket_addr(&self) -> Option<SocketAddr> {
    ...
    let port = unsafe { raw::BIO_ADDR_rawport(self.as_ptr()) }.to_be();

    match family {
      libc::AF_INET => {
        let mut buf = [0u8; 4];
        let mut len = buf.len();
        // SAFETY: `buf` is a valid 4-byte buffer matching `len` on
        // entry; `BIO_ADDR_rawaddress` writes at most `*l` bytes and
        // updates `*l` to the amount actually written, which we check
        // below before trusting `buf`'s contents.
        let ok = unsafe {
      ...
      libc::AF_INET6 => {
        let mut buf = [0u8; 16];
        let mut len = buf.len();
        // SAFETY: same reasoning as the AF_INET arm, 16-byte buffer.
        let ok = unsafe {
          raw::BIO_ADDR_rawaddress(self.as_ptr(), buf.as_mut_ptr() as *mut c_void, &mut len)
        };
        if ok == 0 || len != 16 {
...
pub enum ListenOutcome {
  ...
  /// the sender, if any -- see module SAFETY note #3 for why this is
  /// routinely `None` and must not be relied on as the caller's only source
  /// of the peer address.
...
/// do").
///
/// Takes `&SslRef` (shared), not `&mut`, deliberately: `SslRef::as_ptr()`
/// (via `foreign_types::ForeignTypeRef`) only ever requires `&self` -- the
/// mutation this function causes happens entirely on the C side of the FFI
/// boundary and is invisible to Rust's borrow checker either way, so
/// requiring `&mut` here would just be decorative. This also matches what
/// `openssl::ssl::SslStreamBuilder::ssl()` actually hands back (`&SslRef`),
/// which is the realistic way callers will obtain this reference before a
/// handshake has been driven.
///
...
/// shim assumes (SAFETY note #5). Returns
/// `Err(DtlsShimError::Openssl(..))` for `DTLSv1_listen`'s fatal
/// (`< 0`) return code.
pub fn dtlsv1_listen(ssl: &SslRef) -> Result<ListenOutcome, DtlsShimError> {
