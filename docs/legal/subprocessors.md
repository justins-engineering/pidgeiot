...
| | |
|---|---|
| **Legal entity** | **Snowflake Inc.** The Crunchy Bridge Terms of Service (Last Updated June 27, 2025) are "entered into by and between Snowflake Inc. ("Snowflake") and the entity or person placing an order", and the DPA of the same date defines "Snowflake" as Snowflake Inc. and names it the processor. **Crunchy Data Solutions, Inc. appears nowhere in either document.** It now appears on the Crunchy Bridge sub-processor list as a United States affiliate sub-processor, which is the reverse of its former position. Governing law is Delaware, venue the state and federal courts of New Castle County, Delaware. |
...
| **Data processed** | Fleet and device metadata, shadow mirror, telemetry history, firmware catalogue, alert definitions and state, forwarding endpoints (including their bearer tokens), organisations, members, invitations, business and tax details, billing counters, contact-form submissions, error reports; the Kratos identity database (email addresses, names, phone numbers, credential hashes, sessions, verification and recovery codes, transactional-email log) |
...
| **Service** | Virtual private server hosting the self-hosted identity server (Ory Kratos), its administrative console, the CoAP/DTLS device-transport terminator, and the demo feeder |
| **Data processed** | Identity traffic in transit and in process (registration, login, recovery, settings flows: email addresses, names, credentials, session cookies); device traffic in transit through the terminator (telemetry, shadow reports, log uploads, per-device PSKs fetched from the API); the host's system journal. The identity database itself is on Crunchy Bridge, not on this host. |
...
| | |
|---|---|
| **Legal entity** | Stripe, LLC (United States). Stripe's privacy centre lists "Stripe, LLC" as the contracting entity for "All activities" for United States accounts; the Provider is a US account. Stripe Payments Europe, Limited is not the Provider's contracting entity. |
...
| Entity | Service | Location | Mechanism | Verified | Citation | Fetched |
|---|---|---|---|---|---|---|
