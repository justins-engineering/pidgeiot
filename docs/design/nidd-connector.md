...
- If that transaction fails for a `Nidd` create, or any other step after the DO's 201 does (the
  ...
  DO's own `/pigeon/delete`, as the new owner, and answers 503, so no create that answers 201
  leaves a pigeon over an id whose leftovers were not cleared. This is the one place the
  best-effort mirror rule gives way, and only for this reason; the operator retries. A create the
  DO itself fails after its first INSERT removes its own rows before answering 500. A create whose
  dispatch to the DO fails answers 503 without an undo, so if it landed, its pigeon sits over any
  leftovers until it is deleted (row 42).
...
`_NBIOT_GPS`, or a dual mode preferring NB-IoT); not `CONFIG_TIMER_RANDOM_GENERATOR`, since the
`TELEMETRY` send sequence must start at a random value each boot (7.1, row 44).
